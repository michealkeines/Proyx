...
use crate::protocols::h2::codec::{Codec, SendError, UserError};
use crate::protocols::h2::ext::Protocol;
use crate::protocols::h2::frame::{Headers, Pseudo, Reason, Settings, StreamId};
use crate::protocols::h2::proto::{self, Error};
use crate::protocols::h2::share::{FlowControl, PingPong, RecvStream, SendStream};
    ...
    B: Buf,
{
    ...
    pub fn poll_ready(
        &mut self,
        cx: &mut Context,
    ) -> Poll<Result<(), crate::protocols::h2::error::Error>> {
    ...
    pub fn send_request(
        ...
        request: Request<()>,
        end_of_stream: bool,
    ) -> Result<(ResponseFuture, SendStream<B>), crate::protocols::h2::error::Error> {
        self.inner
...
where
    B: Buf,
{
    type Output = Result<SendRequest<B>, crate::protocols::h2::error::Error>;
...
impl Builder {
    ...
    pub fn handshake<T, B>(
        ...
        Output = Result<(SendRequest<B>, Connection<T, B>), crate::protocols::h2::error::Error>,
    >
    where
...
pub async fn handshake<T>(
    io: T,
) -> Result<(SendRequest<Bytes>, Connection<T, Bytes>), crate::protocols::h2::error::Error>
where
    T: AsyncRead + AsyncWrite + Unpin,
{
...
async fn bind_connection<T>(io: &mut T) -> Result<(), crate::protocols::h2::error::Error>
where
    T: AsyncRead + AsyncWrite + Unpin,
{
    ...
    io.write_all(msg)
        .await
        .map_err(crate::protocols::h2::error::Error::from_io)?;
    ...
    B: Buf,
{
    async fn handshake2(
        mut io: T,
        builder: Builder,
    ) -> Result<(SendRequest<B>, Connection<T, B>), crate::protocols::h2::error::Error> {
    ...
    pub fn set_initial_window_size(
        &mut self,
        size: u32,
    ) -> Result<(), crate::protocols::h2::error::Error> {
        assert!(size <= proto::MAX_WINDOW_SIZE);
...
where
    ...
    B: Buf,
{
    type Output = Result<(), crate::protocols::h2::error::Error>;
...
impl Future for ResponseFuture {
    type Output = Result<Response<RecvStream>, crate::protocols::h2::error::Error>;
...
impl ResponseFuture {
    ...
    pub fn stream_id(&self) -> crate::protocols::h2::share::StreamId {
        crate::protocols::h2::share::StreamId::from_internal(self.inner.stream_id())
    }
...
impl PushPromises {
    ...
    pub async fn push_promise(
        &mut self,
    ) -> Option<Result<PushPromise, crate::protocols::h2::error::Error>> {
        crate::protocols::h2::mac::poll_fn(move |cx| self.poll_push_promise(cx)).await
    }
    ...
    pub fn poll_push_promise(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<Result<PushPromise, crate::protocols::h2::error::Error>>> {
...
impl futures_core::Stream for PushPromises {
    type Item = Result<PushPromise, crate::protocols::h2::error::Error>;
