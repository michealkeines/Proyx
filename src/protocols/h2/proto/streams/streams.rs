...
use crate::protocols::h2::frame::{self, Frame, Reason};
use crate::protocols::h2::proto::{Error, Initiator, Open, Peer, WindowSize, peer};
use crate::protocols::h2::{client, proto, server};
...
impl<B> DynStreams<'_, B> {
    ...
    pub fn send_reset(
        ...
        id: StreamId,
        reason: Reason,
    ) -> Result<(), crate::protocols::h2::proto::error::GoAway> {
        let mut me = self.inner.lock().unwrap();
...
impl<B> StreamRef<B> {
    ...
    pub fn send_reset(&mut self, reason: Reason) {
        ...
        {
            Ok(()) => (),
            Err(crate::protocols::h2::proto::error::GoAway { .. }) => {
    ...
    pub fn send_push_promise(
        ...
        let pushed = {
            ...
            let frame = crate::protocols::h2::server::Peer::convert_push_message(
                stream.id,
                promised_id,
                request,
            )?;
        ...
        counts.transition(stream, |counts, stream| {
            if initiator.is_library() {
                if counts.can_inc_num_local_error_resets() {
                    ...
                    return Err(crate::protocols::h2::proto::error::GoAway {
