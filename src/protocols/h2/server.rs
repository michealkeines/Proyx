...
use crate::protocols::h2::codec::{Codec, UserError};
use crate::protocols::h2::frame::{
    self, Pseudo, PushPromiseHeaderError, Reason, Settings, StreamId,
};
use crate::protocols::h2::proto::{self, Config, Error, Prioritized};
use crate::protocols::h2::share::SendStream;

use crate::protocols::h2::share::*;
use bytes::{Buf, Bytes};
    ...
    B: Buf,
{
    ...
    pub async fn accept(
        &mut self,
    ) -> Option<Result<(Request<RecvStream>, SendResponse<B>), crate::protocols::h2::error::Error>>
    {
        crate::protocols::h2::mac::poll_fn(move |cx| self.poll_accept(cx)).await
    }
    ...
    /// Returns an error if a previous call is still pending acknowledgement
    /// from the remote endpoint.
    pub fn enable_connect_protocol(&mut self) -> Result<(), crate::protocols::h2::error::Error> {
...
where
    ...
    B: Buf,
{
    type Item = Result<(Request<RecvStream>, SendResponse<B>), crate::protocols::h2::error::Error>;
...
impl<B: Buf> SendResponse<B> {
    ...
    pub fn send_response(
        ...
        response: Response<()>,
        end_of_stream: bool,
    ) -> Result<SendStream<B>, crate::protocols::h2::error::Error> {
        self.inner
    ...
    pub fn push_request(
        &mut self,
        request: Request<()>,
    ) -> Result<SendPushedResponse<B>, crate::protocols::h2::error::Error> {
        self.inner
    ...
    pub fn poll_reset(
        &mut self,
        cx: &mut Context,
    ) -> Poll<Result<Reason, crate::protocols::h2::error::Error>> {
...
where
    ...
    B: Buf,
{
    type Output = Result<Codec<T, B>, crate::protocols::h2::error::Error>;
    ...
    B: Buf,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        while rem > 0 {
            ...
            if n == 0 {
                return Poll::Ready(Err(crate::protocols::h2::error::Error::from_io(
                    io::Error::new(
...
where
    ...
    B: Buf,
{
    type Output = Result<Connection<T, B>, crate::protocols::h2::error::Error>;
    ...
    fn convert_poll_message(
        ...
        if has_protocol {
            if is_connect {
                ...
                b = b.extension::<crate::protocols::h2::ext::Protocol>(pseudo.protocol.unwrap());
            } else {
